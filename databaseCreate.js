...
db.page.insert({index: 0, title: 'Objective', name: 'objective.html', path: '/client/views/', description: '<p>This project has been made to show how the author would code a MEAN application from scratch. Main goal is demonstrate some of this stack capabilities, going through angularJS directives, controllers, services, nodeJS modules, mongoDB access and so forth.</p><p>Another thing in mind when it was build, was that should be able to show its own source code by asking the server to read its own system folders and files.</p><p>Every page, including this one, have a "(source code)" link that will show how current page was built.</p>'})
